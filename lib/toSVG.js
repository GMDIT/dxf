...
var text = function text(entity) {
  ...
  /* replace % symbol */
  text = text.replace('%%%', '%');
  var rotation = entity.rotation !== undefined ? ' style="transform-box: fill-box; transform-origin: center left; transform: rotate(' + entity.rotation + 'deg) scale(1,-1);"' : '';
  if (Math.sign(entity.y) > 0) _element = "<text x=\"".concat(entity.x, "\" y=\"").concat(entity.y, "\" class=\"standard\" font-family=\"Arial, sans-serif\" font-size=\"").concat(entity.textHeight + 0.1, "px\" stroke-width=\"0.01%\" transform=\"scale(1, -1) translate(0, -").concat(entity.y * 2, ")\"").concat(rotation, ">").concat(text, "</text>");else _element = "<text x=\"".concat(entity.x, "\" y=\"").concat(entity.y, "\" class=\"standard\" font-family=\"Arial, sans-serif\" font-size=\"").concat(entity.textHeight + 0.1, "px\" stroke-width=\"0.01%\" transform=\"scale(1, -1) translate(0, ").concat(entity.y * -2, ")\"").concat(rotation, ">").concat(text, "</text>");
...
var _default = function _default(parsed) {
  ...
  var _entities$reduce = entities.reduce(function (acc, entity, i) {
      ...
      if (boundsAndElement) {
        ...
        acc.elements.push("<g stroke=\"".concat((0, _rgbToColorAttribute["default"])(rgb), "\" fill=\"").concat(entity.fillColor || (entity.type === 'TEXT' ? (0, _rgbToColorAttribute["default"])(rgb) : ''), "\">").concat(element, "</g>"));
      }
      return acc;
    }, {
